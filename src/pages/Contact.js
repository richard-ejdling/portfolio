...
export default function Contact() {
  ...
  function handleCopy() {
    ...
    navigator.clipboard.writeText("richardej@hotmail.se").then(() => {
      setCopied(true);
  ...
  return (
    <div className="min-h-[calc(100vh-220px)] flex flex-col items-center gap-4 text-xl sm:text-lg">
      <div className="flex flex-col gap-2 sm:flex-row sm:gap-0 items-center">
        ...
        <div
          aria-live="polite"
          className="flex flex-row items-center gap-2 bg-sky-600 pl-1.5 rounded-md"
        >
          richardej@hotmail.se
          <button
            aria-label={"Cypy email adress to clipboard"}
            onClick={handleCopy}
